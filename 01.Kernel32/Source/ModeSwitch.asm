...
; CPUID를 반환
; PARAM: DWORD ax, DWORD *eax, *ebx, *ecx, *edx
ReadCPUID:
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	mov eax, dword [ ebp + 8 ]	; 파라미터 1(dwEAX)을 EAX 레지스터에 저장
	cpuid
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; *eax
	mov esi, dword [ ebp + 12 ]	; 파라미터 2(pwdEAX)를 ESI에 저장
	mov dword [ esi ], eax		; pdwEAX가 포인터이므로 포인터가 가리키는 어드레스에 EAX 레지스터 값 저장

	; *ebx
	mov esi, dword [ ebp + 16 ]	; 파라미터 3(pdwEBX)을 ESI 레지스터에 저장
...
SwitchNExecKernel:
	...
	or eax, 0x0100				; EAX 레지스터에 저장된 IA32_EFER MSR의 하위 32비트에서 LME 비트(비트 8)와 SCE 비트(비트 0)를 1로 설정
	wrmsr						; MSR 레지스터에 쓰기
	...
	or eax, 0xE0000000			; NW 비트(비트 29), CD 비트(비트 30), PG 비트(비트 31), TS 비트(비트 3), EM 비트(비트 2), MP 비트(비트 1)를 모두 1로 설정
	xor eax, 0x60000000			; NW 비트(비트 29), CD 비트(비트 30), EM 비트(비트 2)를 XOR하여 0으로 설정
