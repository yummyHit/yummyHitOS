...
BOOL kInitHDD(void) {
	...
	kOutByte(HDD_PORT_PRIBASE + HDD_PORT_IDX_DIGITOUT, 0);
	kOutByte(HDD_PORT_SECBASE + HDD_PORT_IDX_DIGITOUT, 0);

	// 하드 디스크 정보 요청
	if(kReadHDDInfo(TRUE, TRUE, &(gs_hddManager.hddInfo)) == FALSE) {
	...
	if(kMemCmp(gs_hddManager.hddInfo.modelNum, "QEMU", 4) == 0) gs_hddManager.isWrite = TRUE;
...
static BYTE kReadHDDStat(BOOL pri) {
	if(pri == TRUE) return kInByte(HDD_PORT_PRIBASE + HDD_PORT_IDX_STAT);	// 첫 번째 PATA 포트 상태 레지스터(포트 0x1F7)에서 값 반환
	return kInByte(HDD_PORT_SECBASE + HDD_PORT_IDX_STAT);	// 두 번째 PATA 포트 상태 레지스터(포트 0x177)에서 값 반환
}
...
static BOOL kWaitHDDBusy(BOOL pri) {
	...
	// 대기를 시작한 시간 저장
	startTickCnt = kGetTickCnt();

	// 일정 시간 동안 하드 디스크의 Busy가 해제될 때까지 대기
	while((kGetTickCnt() - startTickCnt) <= HDD_WAIT_TIME) {
		// HDD 상태 반환
...
static BOOL kWaitHDDInterrupt(BOOL pri) {
	...
	while((kGetTickCnt() - tickCnt) <= HDD_WAIT_TIME) {
...
BOOL kReadHDDInfo(BOOL pri, BOOL master, HDDINFO *hddInfo) {
	...
	// 동기화 처리
	kLock(&(gs_hddManager.mut));

	// 아직 수행중 커맨드가 있다면 일정 시간동안 끝나길 대기
	if(kWaitHDDBusy(pri) == FALSE) {
		// 동기화 처리
	...
	kOutByte(portBase + HDD_PORT_IDX_DRIVENHEAD, driveFlag);

	// 커맨드 받아들일 준비가 될 때까지 일정 시간 대기
	if(kWaitHDDReady(pri) == FALSE) {
		// 동기화 처리
		kUnlock(&(gs_hddManager.mut));
		return FALSE;
	}
	...
	kOutByte(portBase + HDD_PORT_IDX_CMD, HDD_CMD_ID);

	// 처리가 완료될 때까지 인터럽트 발생 대기
	waitRes = kWaitHDDInterrupt(pri);
	...
	// 한 섹터 읽음
	for(i = 0; i < 512 / 2; i++) ((WORD*)hddInfo)[i] = kInWord(portBase + HDD_PORT_IDX_DATA);

	// 문자열은 바이트 순서로 다시 변환
	kSwapByte(hddInfo->modelNum, sizeof(hddInfo->modelNum) / 2);
	kSwapByte(hddInfo->serialNum, sizeof(hddInfo->serialNum) / 2);
...
int kReadHDDSector(BOOL pri, BOOL master, DWORD lba, int sectorCnt, char *buf) {
	...
	kOutByte(portBase + HDD_PORT_IDX_CMD, HDD_CMD_READ);
	...
	for(i = 0; i < sectorCnt; i++) {
		...
		// 한 섹터 읽음
		for(j = 0; j < 512 / 2; j++) ((WORD*)buf)[readCnt++] = kInWord(portBase + HDD_PORT_IDX_DATA);
	}
...
int kWriteHDDSector(BOOL pri, BOOL master, DWORD lba, int sectorCnt, char *buf) {
	...
	if(kWaitHDDBusy(pri) == FALSE) return FALSE;
	...
	// 커맨드 전송
	kOutByte(portBase + HDD_PORT_IDX_CMD, HDD_CMD_WRITE);
	...
	for(i = 0; i < sectorCnt; i++) {
		...
		kSetHDDInterruptFlag(pri, FALSE);
		for(j = 0; j < 512 / 2; j++) kOutWord(portBase + HDD_PORT_IDX_DATA, ((WORD*)buf)[readCnt++]);
