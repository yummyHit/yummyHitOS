...
BOOL initRDD(DWORD sectorCnt) {
	// 자료구조 초기화
	memSet(&gs_rddManager, 0, sizeof(gs_rddManager));

	// 램 디스크로 사용할 메모리 할당
	gs_rddManager.buf = (BYTE*)allocMem(sectorCnt * 512);
	if(gs_rddManager.buf == NULL) return FALSE;

	// 총 섹터 수와 동기화 객체 설정
	gs_rddManager.totalSectorCnt = sectorCnt;
	initMutex(&(gs_rddManager.mut));
...
BOOL readRDDInfo(BOOL pri, BOOL master, HDDINFO *hddInfo) {
	...
	memCpy(hddInfo->serialNum, "0000-0000", 9);
	memCpy(hddInfo->modelNum, "YummyHitOS RAM Disk v1.0", 24);
...
int readRDDSector(BOOL pri, BOOL master, DWORD lba, int sectorCnt, char *buf) {
	...
	memCpy(buf, gs_rddManager.buf + (lba * 512), realCnt * 512);
...
int writeRDDSector(BOOL pri, BOOL master, DWORD lba, int sectorCnt, char *buf) {
	...
	memCpy(gs_rddManager.buf + (lba * 512), buf, realCnt * 512);
