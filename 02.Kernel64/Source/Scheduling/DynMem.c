...
void initDynMem(void) {
	...
	blockCnt = calcMetaBlockCnt(memSize);

	// 전체 블록 개수에서 관리에 필요한 메타블록 개수를 제외한 나머지 영역에 대해 메타 정보 구성
	gs_dynmem.smallBlockCnt = (memSize / DYNMEM_MIN_SIZE) - blockCnt;
	...
	gs_dynmem.startAddr = DYNMEM_START_ADDR + blockCnt * DYNMEM_MIN_SIZE;
...
static BOOL freeBuddyBlock(int blockIdx, int offset) {
	...
	for(i = blockIdx; i < gs_dynmem.maxLvCnt; i++) {
		...
		if((offset % 2) == 0) blockOffset = offset + 1;
		else blockOffset = offset -1;
		flag = getFlagBitmap(i, blockOffset);

		// 블록이 비어있으면 종료
		if(flag == DYNMEM_EMPTY) break;

		// 여기까지 왔으면 인접 블록이 있으니 블록 결합. 블록을 모두 비우고 상위 블록으로 이동
		setFlagBitmap(i, blockOffset, DYNMEM_EMPTY);
