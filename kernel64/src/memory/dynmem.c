...
void kInitDynMem(void) {
	...
	memSize = kCalcDynMemSize();
	metaCnt = kCalcMetaBlockCnt(memSize);
	...
	gs_dynMem.endAddr = kCalcDynMemSize() + DYNMEM_START_ADDR;
	gs_dynMem.usedSize = 0;

	// 스핀락 초기화
	kInitSpinLock(&(gs_dynMem.spinLock));
}
...
static QWORD kCalcDynMemSize(void) {
	...
	memSize = (kGetTotalMemSize() * 1024 * 1024);
...
void *kAllocMem(QWORD size) {
	...
	alignSize = kGetBuddyBlockSize(size);
	...
	offset = kAllocBuddyBlock(alignSize);
	if(offset == -1) return NULL;

	blockIdx = kGetMatchSizeIdx(alignSize);
...
static int kAllocBuddyBlock(QWORD alignSize) {
	...
	if(i > blockIdx) {
		...
		for(i = i - 1; i >= blockIdx; i--) {
			// 왼쪽 블록
			kSetFlagBitmap(i, freeOffset * 2, DYNMEM_EMPTY);
			// 오른쪽 블록
			kSetFlagBitmap(i, freeOffset * 2 + 1, DYNMEM_EXIST);
			// 왼쪽 블록 분할
...
BOOL kFreeMem(void *addr) {
	...
	if(kFreeBuddyBlock(blockIdx, bitmapOffset) == TRUE) {
...
void kGetDynMemInfo(QWORD *dynMemStartAddr, QWORD *dynMemTotalSize, QWORD *metaDataSize, QWORD *usedMemSize) {
	...
	*dynMemTotalSize = kCalcDynMemSize();
	*metaDataSize = kCalcMetaBlockCnt(*dynMemTotalSize) * DYNMEM_MIN_SIZE;
