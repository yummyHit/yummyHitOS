...
int kAlignFormatting(int alignLen, char *buf, int *bufIdx) {
	int i;
...
int kVsprintf(char *buf, const char *format, va_list v) {
	QWORD i, j, k, qwV;
	int bufIdx = 0, fmLen, cpLen, iV, alignLen, dotLen;
	char *cpStr;
	BOOL align = FALSE, dot = FALSE;
	double dV;
	...
	for(i = 0; i < fmLen; i++) {
		if(format[i] == '%') {
			...
			if(format[i] >= '0' && format[i] <= '9') {
				for(alignLen = 0, j = 0; format[i] >= '0' && format[i] <= '9'; i++, j++) {
					if(j > 0) alignLen *= 10;
					alignLen += format[i] - '0';
				}
				if(!align) kAlignFormatting(alignLen, buf, &bufIdx);
			}
			...
			if(dot && format[i] >= '0' && format[i] <= '9')
				for(dotLen = 0, j = 0; format[i] >= '0' && format[i] <= '9'; i++, j++) {
					if(j > 0) dotLen *= 10;
					dotLen += format[i] - '0';
				}
