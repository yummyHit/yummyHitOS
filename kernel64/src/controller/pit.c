...
void kInitPIT(WORD cnt, BOOL term) {
	...
	kOutByte(PIT_PORT_CTRL, PIT_COUNTER_ZERO_ONCE);

	// 만약 일정 주기로 반복하는 타이머라면 모드 2로 설정
	if(term == TRUE) kOutByte(PIT_PORT_CTRL, PIT_COUNTER_ZERO_TERM); // PIT 컨트롤 레지스터(포트 0x43)에 모드 2에 바이너리 카운터 설정

	// 카운터 0(포트 0x40)에 LSB->MSB 순서로 카운터 초기 값 설정
	kOutByte(PIT_PORT_COUNTER_ZERO, cnt);
	kOutByte(PIT_PORT_COUNTER_ZERO, cnt >> 8);
}
...
WORD kReadCntZero(void) {
	...
	kOutByte(PIT_PORT_CTRL, PIT_COUNTER_ZERO_LATCH);

	// 카운터 0(포트 0x40)에서 LSB->MSB 순서로 카운터 값 읽음
	low = kInByte(PIT_PORT_COUNTER_ZERO);
	high = kInByte(PIT_PORT_COUNTER_ZERO);
